 ...
 * thus the largest prime that I can affirm for you is prime is the greatest prime that is smaller than the greatest prime in the prime table.
...
unsigned long prime(unsigned long potential_prime, char *prime_table_filename, int binary_mode) {
	...
	return 3; // Error code 4 --> 'printf("Failed to open file containing prime table.\n"); return -1;'
...
int main(int argc, char *argv[]) {
    if (argc != 2) {
	return -1;
    } fprintf(stdout, "Hello, I'm %s, I check for primality using a prime table to look up potential prime divisors quickly.\n\n", argv[0]);
