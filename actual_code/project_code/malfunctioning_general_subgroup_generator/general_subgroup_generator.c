/* I am content with this implementation */
#include <stdio.h>
#include <stdlib.h>
...
void *disintermediat_channel(void **to_be_disintermediated) {
...
void vertibrae_input(struct vertibrae **tracer, unsigned long new_ulong, unsigned cell_width) {
...
struct permutation_piece *yield_subgroup(struct vertibrae *upstream_link, struct permutation_piece **source, struct group_properties *group) {
    ...
    last_element = first_element = (struct permutation_piece *) disintermediat_channel((void **) source);
...
struct vertibrae *print_table(struct vertibrae *initial_row, struct table_properties *table_properties) {
	...
	printf("<%s> = {", do_loop_iterator->unit.ascii);
...
struct vertibrae *setup_table(struct vertibrae *identity_element, struct group_properties *group, unsigned long *cardinality) {
	...
	do_loop_iterator->permutation = yield_subgroup(do_loop_iterator, (struct permutation_piece **) channel_source(), group);
	(*cardinality)++;
	do_loop_iterator = do_loop_iterator->next;
...
struct vertibrae *build_backbone(struct vertibrae **source_channel, struct group_properties *group) {
    /* ### Create table ### */
	...
	if (group->identity == ADDITIVE_IDENTITY || coprime(GCD(group->modulus, element)))
	    vertibrae_input(source_channel, element, cell_width);
    } while (element != group->identity);

    /* ### Make circular ### */
    struct vertibrae *last_element, *first_element;
    last_element = first_element = (struct vertibrae *) disintermediat_channel((void **) source_channel);
    while (last_element->next) {
	last_element = last_element->next;
    } last_element->next = first_element;
...
int main(int argc, char **argv) {
    ...
    struct vertibrae *table = setup_table(build_backbone((struct vertibrae **) channel_source(), group), group, &cardinality);
    print_table(table, table_properties);
    free_table(table);
    fprintf(fs, "\n");

    if (group->identity) { // Display cardinality of multiplicative group of integers
	fprintf(fs, "|<\u2124/%lu\u2124, *>| = %lu", group->modulus, cardinality);
