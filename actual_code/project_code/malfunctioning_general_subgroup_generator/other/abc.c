...
void vertibrae_insert(struct vertibrae **tracer, unsigned long new_ulong, unsigned cell_width) {
    ...
    new_vertibrae->unit.ascii = ul_to_str(new_long, cell_width);
...
struct permutation_piece *yield_subgroup(struct vertibrae *upstream_link, struct permutation_piece **tracer, struct group_properties *group) {
    ...
    last_element = first_element = (struct permutation_piece *) close_source((void **) tracer);
...
void print_row(struct permutation_piece *identifier_shackle, unsigned long horizontal_offset) {
	...
	fprintf(fs, "%lu", do_loop_iterator->unit->number);
...
struct vertibrae *print_table(unsigned long *group_cardinality, struct vertibrae *initial_row, struct set_of_table_properties *table_properties) {
...
struct vertibrae *setup_table(struct vertibrae *identity_element, struct group_properties *group_parameters) {
    do {
	do_loop_iterator->permutation = yield_subgroup(do_loop_iterator, (struct permutation_piece **) open_source(), group_parameters);
	printf("<%s^(-1)> = {", do_loop_iterator->unit.ascii);
	print_row(do_loop_iterator->permutation, 0); printf("}\n");
	do_loop_iterator = do_loop_iterator->next;
    } while (do_loop_iterator != last_element);
    return do_loop_iterator; // <=== Return linked list at the identity element =====###
}

struct vertibrae *build_backbone(struct vertibrae **tracer, struct group_properties *group) {
    /* ### Create table ### */
    unsigned long element = group->modulus;
    unsigned long cell_width = element - 1;
    do {
	element--;
	if (group->identity == ADDITIVE_IDENTITY || coprime(GCD(group->modulus, element)))
	    vertibrae_insert(tracer, element, cell_width);
    } while (element != group->identity);

    /* ### Make circular ### */
    struct vertibrae *last_element, *first_element;
    last_element = first_element = (struct vertibrae *) close_source((void **) tracer);
    while (last_element->next) {
	last_element = last_element->next;
    } last_element->next = first_element;
...
int main(int argc, char **argv) {
    struct set_of_table_properties *table_properties = malloc(sizeof(struct set_of_table_properties));
    struct group_properties *group_parameters = malloc(sizeof(struct group_properties));
    switch (argc) {
	case 6:
	    fs = fopen(argv[5], "w");
	case 5:
	...
	case 3:
	    group_parameters->identity = string_to_unsigned_long(argv[2]);
	case 2:
	...
	fscanf(stdin, "%lu", &group_parameters->identity); } // Take in the group identity.
    ...
    } table_properties->horizontal_offset %= group_parameters->modulus;

    unsigned long cell_width;
    unsigned long group_cardinality = 0;

    setup_table(build_backbone((struct vertibrae **) open_source(), group_parameters), group_parameters);

    fprintf(fs, "\n");
    if (group_parameters->identity) { // Display cardinality of multiplicative group of integers
