...
struct permutation_piece *yield_subgroup(struct vertibrae *upstream_link, struct permutation_piece **tracer, struct set_of_group_parameters *group) {
    ...
    last_element = first_element = (struct permutation_piece *) disintermediate((void **) tracer);
...
struct vertibrae *setup_table(struct vertibrae *last_element, struct set_of_group_parameters *group_parameters) {
    ...
    /* ### Make associative linked list complete (printable) ### */
    struct vertibrae *do_loop_iterator = last_element; do {
	do_loop_iterator->unit.ascii = ul_to_str(do_loop_iterator->unit.number, cell_width);
	do_loop_iterator = do_loop_iterator->next;
    } while (do_loop_iterator != last_element);

    do {
	do_loop_iterator->permutation = yield_subgroup(do_loop_iterator, permutation_needle_insert(), group_parameters);
...
struct vertibrae *build_backbone(struct vertibrae **tracer, struct set_of_group_parameters *group) {
    ...
    last_element = first_element = (struct vertibrae *) disintermediate((void **) tracer);
    while (last_element->next) {
	last_element = last_element->next;
    } last_element->next = first_element;
...
int main(int argc, char **argv) {
	...
	print_table(
	    ...
	    setup_table(build_backbone(vertibrae_needle_insert(), group_parameters), group_parameters);/* ,
	    table_parameters
	    )
	); */
