...
void main(int argc, char *argv[]) {
    ...
    for (unsigned long generating_element = 2; generating_element < modulus; generating_element++) {
	if (gcd(modulus, generating_element) == 1) {
	    number_of_elements++;
	    unsigned long iterator = generating_element;
	    printf("<%lu> = {%lu, ", generating_element, MULTIPLICATIVE_IDENTITY);
	    while (((iterator * generating_element) % modulus) != MULTIPLICATIVE_IDENTITY) {
		printf("%lu, ", iterator);
