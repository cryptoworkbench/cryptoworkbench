...
unsigned long group_operation(ul a, ul b) { return (*__group[retrieve_current_group()]->oper)(a, b, __group[retrieve_current_group()]->mod); }
unsigned long group_conditional_cap(ul result) { return (__group[retrieve_current_group()]->mod) ? _conditional_cap(result, __group[retrieve_current_group()]->mod) : result; }
unsigned long group_add(ul a, ul b) { return _add(a, b, __group[retrieve_current_group()]->mod); }
unsigned long group_inverse(ul element_of_additive_group) { return _inverse(element_of_additive_group, __group[retrieve_current_group()]->mod); }
unsigned long group_subtract(ul a, ul b) { return _subtract(a, b, __group[retrieve_current_group()]->mod); }
unsigned long group_multiply(ul a, ul b) { return _conditional_cap(a * b, __group[retrieve_current_group()]->mod); } 
unsigned long group_divide(ul numerator, ul denominator) { return _divide(numerator, denominator, __group[retrieve_current_group()]->mod); }
unsigned long group_exponentiate(ul base, ul exponent) { return _exponentiate(base, exponent, __group[retrieve_current_group()]->mod); }
unsigned long group_polynomial(ul x, ul_ptr coefficient, int number_of_coefficients) { return _polynomial(x, coefficient, number_of_coefficients, __group[retrieve_current_group()]->mod); }
/* ======= ^ = ^ == these wrapper functions insert the modulus value which is store in the sloth 'mod' from struct group. == ^ = ^ ======= */
...
{ 
    ...
    const char *operation_symbol = __group[retrieve_current_group()]->sign[1];
    const char *adjective = __group[retrieve_current_group()]->sign[4];
    const char *group_ID = __group[retrieve_current_group()]->sign[0];
...
void close_group(char *mod, FILE *opened_group)
{
    ...
    sprintf(BUFFER, "Sourced \u2115%s%s from '%s'", mod, __group[retrieve_current_group()]->sign[1], path_to_group); append_to_LOGBOOK(BUFFER); fclose(opened_group);
