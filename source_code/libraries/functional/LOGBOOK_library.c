...
FILE *open_modular_GROUP_in_the_NAME_of(struct group_prams *GROUP, char *prog_NAME, char **path_to_file_INSERTMENT_SLOTH) {
	...
	sprintf(BUFFER, "Failed to secure a filestream sourced by '%s' \u21D2 the group denoted <\u2124/%lu\u2124, %s> does not seem to already have been registered", PATH_TO_FILE, GROUP->CAP, symbol); FLUSH_TO_FS(prog_NAME, BUFFER);
	...
	sprintf(BUFFER, "Sending '%s' to the operating system in an effort to manually register <\u2124/%lu\u2124, %s>", required_command, GROUP->CAP, symbol); FLUSH_TO_FS(prog_NAME, BUFFER);
