...
int main(int argc, char **argv) { mod = numerator = denominator = ADDITIVE_IDENTITY; _mod = &mod;
    ...
    if (numerator >= mod || mod <= denominator) fprintf(stderr, "%lu / %lu \u2261 ", numerator, denominator); _mod = &mod; numerator = mod_conditional_cap(numerator);
