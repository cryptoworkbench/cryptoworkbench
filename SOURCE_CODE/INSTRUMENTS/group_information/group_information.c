...
struct _LL **establish_LL(char **argv, group_OBJ group) {
    struct _CHANNEL_PTR_pair element_CHANNEL_PTR_pair; INITIALIZE_CHANNEL_PTR_pair(&element_CHANNEL_PTR_pair.iterator, &element_CHANNEL_PTR_pair.head);
...
struct _LL *replace_LL_with_table(struct _LL **element_CHANNEL, group_OBJ group) {
    ...
    struct _CHANNEL_PTR_pair generator_CHANNEL_PTR_pair; INITIALIZE_CHANNEL_PTR_pair(&generator_CHANNEL_PTR_pair.iterator, &generator_CHANNEL_PTR_pair.head); // << Declare new pointers and perform the same magic but this time in order to create a list of generators
