...
void field_combination(struct ECC_point *P, struct ECC_point *Q, struct ECC_point *R) {
    ...
    if (P->x == Q->x && P->y == Q->y) {
	m = (((((FINITE_N_exponentiation(P->x, 2, MOD) * 3) % MOD) + A) % MOD) * multiplicative_inverse((P->y * 2) % MOD, MOD)) % MOD; // Formula for point doubling
    ...
    unsigned long new_x = (((FINITE_N_exponentiation(m, 2, MOD) + (MOD - P->x)) % MOD) + (MOD - P->x)) % MOD; 
...
struct ECC_point *scalar_array(struct ECC_point *Base_Point, unsigned long cardinality) {
	...
	printf("2^%luK = %luK = (%lu, %lu)\n", index, N_exponentiation(2, index), point_array[index].x, point_array[index].y, index);
...
int main(int argc, char **argv) { fprintf(stdout, "Domain parameters:\n");
    ...
    multiplier -= N_exponentiation(2, least_base_two_logarithm); // <<< Calculate updated multiplier
