...
int main(int argc, char **argv) {
    if (2 > argc || !str_represents_ul(argv[1], &_mod)) argv_ERROR(argv, 1); unsigned long number_coprime_to_MOD;
    if (3 > argc || !str_represents_ul(argv[2], &number_coprime_to_MOD)) argv_ERROR(argv, 2); unsigned long GCD_of_arguments = GCD(number_coprime_to_MOD, _mod);
    if (GCD_of_arguments != 1) { fprintf(stderr, "%lu shares a factor of %lu with %lu.\n\n" EXIT_STATUS_GOODBYE, number_coprime_to_MOD, GCD_of_arguments, _mod, -3); exit(-3); }
    fprintf(stdout, "%lu^-1 \u2261 %lu	(mod %lu)\n", number_coprime_to_MOD, mod_division(1, number_coprime_to_MOD), _mod); return 0;
}
