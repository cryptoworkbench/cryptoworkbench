...
int main(int argc, char **argv) { unparsed_arg = argv[1]; unsigned long mod; mod_ = &mod; 
    conditional_goodbye(n(n(error_specification(mod_error, n(str_represents_ul(unparsed_arg, mod_, -1)))))); unparsed_arg = argv[2]; unsigned long a;
    conditional_goodbye(n(n(error_specification(a_error, n(str_represents_ul(unparsed_arg, &a, -2))))));
    // take in variables ^
