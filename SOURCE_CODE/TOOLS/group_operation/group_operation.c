...
unsigned long mod; int main(int argc, char **argv) { mod_ = &mod; local_error_selector = local_switch;
    if (!str_represents_ul(argv[1], mod_)) arg_does_not_represent_ul(argv, 1); unsigned long a;
    if (!str_represents_ul(argv[2], &a)) arg_does_not_represent_ul(argv, 2); unsigned long b;
    if (!str_represents_ul(argv[3], &b)) arg_does_not_represent_ul(argv, 3); unsigned long ID;
    if (!match_variadic(argv[4], 2, "0", "1")) arg_does_not_represent_ul(argv, 4); str_represents_ul(argv[4], &ID);
