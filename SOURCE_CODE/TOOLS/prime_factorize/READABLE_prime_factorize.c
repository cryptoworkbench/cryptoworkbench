...
struct _crux { unsigned long *prime_factor; int *log; int number_of_distinct_prime_factors; };
// program crux ^
...
struct _crux *CRUX_allocate(int number_of_distinct_prime_factors) {
	...
	(int *) malloc(sizeof(int) * number_of_distinct_prime_factors); for (int i = 0; i < number_of_distinct_prime_factors; i++) ret_val->log[i] = 1; ret_val->number_of_distinct_prime_factors =
	number_of_distinct_prime_factors; return ret_val;
...
{ if (index == crux->number_of_distinct_prime_factors) return multiplicative_accumulator; return CRUX_calculate(multiplicative_accumulator * exponentiate(crux->prime_factor[index], crux->log[index]), crux, 1 + index); }
...
{ fprintf(stdout, "%lu^%lu", crux->prime_factor[index], crux->log[index]); index++; if (index == crux->number_of_distinct_prime_factors) return crux; fprintf(stdout, " * "); return _CRUX_print(crux, index); }
