...
int main(int argc, char **argv) { unsigned long composite; unparsed_arg = argv[1];
    ...
    struct LL_ *divisors = (struct LL_ *) malloc(sizeof(struct LL_)); divisors->e = composite; divisors->next = NULL; /* struct LL_ *tail = */ divisor_list_stretch(divisors);
    PRIME_FACTORIZATION_free(PRIME_FACTORIZATION_print(PRIME_FACTORIZATION_initialize(number_of_distinct_prime_factors(divisors->e, divisors->next, 1), divisors)));
    return 0;
}
