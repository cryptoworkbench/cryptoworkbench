...
int main(int argc, char **argv) {
    ...
    fprintf(stdout, "Exported %lu primes to external file '%s' (which is all primes less than %lu).\n", primes_printed_from_sieve_array_to_FS(sieve_of_eratosthenes(REPORT_MOD()), REPORT_MOD(), FS_to_external_prime_table), output_filename, REPORT_MOD()); fclose(FS_to_external_prime_table); return 0;
}
