...
_factorization_method initialize(unsigned long composite) { _factorization_method preferred_factorization_ENGINE = _REPORT_preferred_factorization_ENGINE();
    if (preferred_factorization_ENGINE == LEAST_efficient_trial_division) { fprintf(stdout, "Using trial mod_division"); domain_display(trial_limit(composite, 3), composite); }
    else if (preferred_factorization_ENGINE == LESS_efficient_trial_division) { fprintf(stdout, "Using trial mod_division"); domain_display(trial_limit(composite, 2), composite); }
    else if (preferred_factorization_ENGINE == efficient_trial_division) { fprintf(stdout, "Using trial mod_division"); domain_display(trial_limit(composite, 1), composite); }
...
int main(int argc, char **argv) { unsigned long composite;
    ...
    struct ordered_pair factor_a_and_b = factorize(composite, NULL); // <- NULL gives the same just as if '_REPORT_preferred_factorization_ENGINE()' was passed as second argument to this function
