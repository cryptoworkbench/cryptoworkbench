...
unsigned long mod; int main(int argc, char **argv) { mod_ = &mod; local_error_selector = local_switch;
    if (!str_represents_ul(argv[1], mod_)) str_does_not_represent_suitable_ul(argv, 1); unsigned long inverse_of_additve_inverse;
    if (!str_represents_ul(argv[2], &inverse_of_additve_inverse)) str_does_not_represent_suitable_ul(argv, 2);
