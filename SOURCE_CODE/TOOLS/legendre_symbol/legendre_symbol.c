...
int main(int argc, char **argv) { const char *specific_file = _REPORT_preferred_factorization_engine_file();
    ...
    int SELECTOR = translate_SUBTRACT_ONE(BUFFER); if (SELECTOR) SET_preferred_factorization_ENGINE(SELECTOR - 1); else FACTORIZATION_METHOD_UNCHOSEN(BUFFER); free(BUFFER);
