...
int main(int argc, char **argv) { const char *specific_file = _REPORT_preferred_factorization_engine_file();
    ...
    int SELECTOR = SELECTOR_from_str_representing_factorization_method(BUFFER); if (SELECTOR) preferred_factorization_ENGINE = factorization_method(SELECTOR - 1); else FACTORIZATION_METHOD_UNCHOSEN(BUFFER); free(BUFFER);
