...
void argument_parse() { fprintf(stderr, "Failed to parse '%s'!", (*argv_ptr)[1 + i]); };
...
int main(int argc, char **argv) { argv_ptr = &argv;
    ...
    conditional_goodbye(n(n(error_specification(argument_parse, n(_ul_parse_str(i + arguments_array, argv[1 + i], -(2 + i))))))); i = 0;
