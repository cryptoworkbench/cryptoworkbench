...
void number_failed_to_parse() { fprintf(stderr, "Please provide as first argument the number to factorize. '%s' is not a number!", (*argv_loc)[1]); }
// error functions ^

int main(int argc, char **argv) { number = ADDITIVE_IDENTITY; argv_loc = &argv;
