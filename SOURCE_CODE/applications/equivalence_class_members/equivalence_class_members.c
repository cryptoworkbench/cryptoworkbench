...
int main(int argc, char **argv) { argv_ptr = &argv;
    unsigned long N_quotient; conditional_goodbye(n(n(error_specification(quotient_group_failed_to_parse, n(_ul_parse_str(argv[1], &N_quotient, -1))))));
    unsigned long a;          conditional_goodbye(n(n(error_specification(a_error, n(_ul_parse_str(argv[2], &a, -2))))));
    int amount;               conditional_goodbye(n(n(error_specification(a_error, n(_ul_parse_str(argv[3], &amount, 0))))));
    // take in variables ^
