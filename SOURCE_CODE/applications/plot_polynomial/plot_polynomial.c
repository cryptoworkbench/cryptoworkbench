...
int main(int argc, char **argv) { mod = ADDITIVE_IDENTITY;
    conditional_goodbye(n(n(error_specification(________mod_failed_to_parse, n(ul_parse_str(&mod, argv[1], -1)))))); mod_ = &mod;
    ...
    conditional_goodbye(n(n(error_specification(coefficient_failed_to_parse, n(ul_parse_str(coefficient + i, argv[i + 2], -3)))))); i = 0;
