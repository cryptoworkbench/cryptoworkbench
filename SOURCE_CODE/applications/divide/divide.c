...
int main(int argc, char **argv) {
    unsigned long         mod = 0; conditional_goodbye(n(n(error_specification(___quotient_failed_to_parse, n(_ul_parse_str(&        mod, argv[1], -1))))));
    unsigned long   numerator = 0; conditional_goodbye(n(n(error_specification(__numerator_failed_to_parse, n(_ul_parse_str(&  numerator, argv[2], -2))))));
    unsigned long denominator = 0; conditional_goodbye(n(n(error_specification(denominator_failed_to_parse, n(_ul_parse_str(&denominator, argv[3], -3))))));
