...
int string_to_int(char *string) {
	...
	string_as_integer += (*current_character - ASCII_BASE) * _exponentiate(NUMERIC_BASE, iteration_count, 0);

	iteration_count++; // Update the iteration count
	current_character = (current_character - 1); // Move back one character
    } string_as_integer += (*current_character - ASCII_BASE) * _exponentiate(NUMERIC_BASE, length_of_string - 1, 0);
...
unsigned long char_in_val(unsigned long a) {
    ...
    char_index = 0; intermediate_value = _exponentiate(NUMERIC_BASE, char_index, 0);

    while (!(intermediate_value >= a)) /* Calculate the first power of NUMERIC_BASE which is greater than or equal to a */
    { char_index++; intermediate_value = _exponentiate(NUMERIC_BASE, char_index, 0); }
