...
unsigned long **equation_NEGATIVE(unsigned long **equation) {
    unsigned long **resulting_equation = UL_array_of_SIZE(UL_array_LENGTH(equation));
...
unsigned long equation_multiplier(unsigned long **equation_ONE, unsigned long **equation_TWO, int index) {
    int LENGTH_OF_equation_ONE = UL_array_LENGTH(equation_ONE); if (LENGTH_OF_equation_ONE != UL_array_LENGTH(equation_TWO)) exit(-1);
