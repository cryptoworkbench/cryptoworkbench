...
unsigned long **equations_ALLOCATE(int K) {
    ...
    for (int i = 0; i < K; i++) ret_val[i] = UL_array_of_SIZE(K);
    return ret_val;
}
...
void equations_DELETE(unsigned long **equation, int amount) { for (int i = 0; i < amount; i++) free(equation[i]); free(equation); }
