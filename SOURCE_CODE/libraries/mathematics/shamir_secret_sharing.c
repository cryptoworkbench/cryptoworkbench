...
void SET_k(int length_minus_two)
{
    k = length_minus_two;
}

int k_yield() { return k; }
unsigned long *equation_ADD(unsigned long *first_equation, unsigned long *second_equation)
{
    unsigned long *ret_val = (unsigned long *) malloc(sizeof(unsigned long) * (k + 2));
    for (int i = 0; i < k + 2; i++) ret_val[i] = mod_add(first_equation[i], second_equation[i]);
    return ret_val;
} // ^ dep of 'equation_SUBTRACT()'

unsigned long *equation_MULTIPLY(unsigned long *equation, unsigned long multiplier)
{
    unsigned long *ret_val = UL_array_of_SIZE(k + 2);
    for (int i = 0; i < k + 2; i++) ret_val[i] = mod_multiply(equation[i], multiplier); return ret_val;
}

unsigned long *equation_NEGATIVE(unsigned long *equation)
{
    unsigned long *ret_val = UL_array_of_SIZE(k + 2);
    for (int i = 0; i < k + 2; i++) ret_val[i] = mod_inverse(equation[i]);
    return ret_val;
} // ^ dep of 'equation_SUBTRACT()'
...
{ unsigned long *ret_val;
    if (first_equation[index] != second_equation[index]) ret_val = equation_MULTIPLY(first_equation, mod_divide(first_equation[index], mod_inverse(second_equation[index])));
    else ret_val = equation_SUBTRACT(first_equation, second_equation); return ret_val;
}
