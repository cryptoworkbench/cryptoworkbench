...
int fermat_primality_test_DETERMINISTIC(unsigned long potential_prime) {
    for (ul i = 2; i < potential_prime; i++) {
	unsigned long _GCD = GCD(i, potential_prime); if (_GCD > 1) return 0; // < potential_prime turns out to actually be composite
	unsigned long raised = exponentiation(i, potential_prime - 1); if (raised > 1) return 0; // < potential_prime turns out to actually be composite
    } return 1;
}
